...
import { ensureTerminalElement } from '../reconciler/operations'
...
export function renderComponent(
  ...
  const rootElement = createElement('box', {
    width: '100%',
    height: '100%',
    border: options.debug ? true : false,
  });
  ...
  try {
    ...
    import('svelte').then(async (svelteModule) => {
      try {
        ...
        if (typeof component === 'function' && !component.__svelte_component__) {
          // This might be a direct DOM component (not a Svelte component)
          try {
            if (options.debug) {
              console.log('[Renderer] Component appears to be a direct DOM function');
            }
            
            // Call the function to get a DOM node
            const domResult = component(options.props || {});
            
            if (domResult && domResult.nodeType) {
              if (options.debug) {
                console.log('[Renderer] Component returned a DOM node');
              }
              
              // If the function returned a DOM node, append it to our root
              rootNode.appendChild(domResult);
              ...
              instance = { 
                ...
                unmount: () => {
                  try {
                    rootNode.removeChild(domResult);
                  } catch (err) {
            ...
            console.error('[Renderer] Error handling DOM component:', domError);
        ...
        if (options.debug) {
          console.log('[Renderer] Trying Svelte component mounting');
          console.log('[Renderer] Available Svelte exports:', Object.keys(svelteModule));
        }
        
        // Handle component as a string (path) or function (component class)
        if (svelteModule.mount) {
          // Svelte 5 API
          if (options.debug) {
          ...
          const mountComponent = async () => {
            ...
            if (typeof component === 'string') {
              ...
              try {
                ...
                componentToMount = module.default;
            ...
            return svelteModule.mount(componentToMount, {
              target: rootNode,
              props: options.props || {}
            });
          };
          ...
          mountComponent()
            .then(mountedInstance => {
              ...
              // Process the resulting DOM using the reconciler
              if (options.debug) {
                console.log('[Renderer] Component mounted, processing DOM tree with reconciler');
              }
  ...
  const cleanup = () => {
    try {
      ...
      // Get the Svelte module to unmount the component
      if (instance) {
        // Dynamic import to get the unmount function
        import('svelte').then(svelteModule => {
          if (svelteModule.unmount) {
            // Svelte 5 API
            if (options.debug) {
              console.log('[Renderer] Using Svelte 5 unmount API');
            }
            svelteModule.unmount(instance);
 ...
 */
function processComponentWithReconciler(node: TerminalNode, parent: TerminalElement): void {
  ...
  if (node.nodeType === 1) {
    ...
    if (!elementNode._terminalElement) {
      ...
      parent.attachToNode(elementNode);
    }
  }
 ...
 */
function createPlaceholderUI(rootElement: TerminalElement, screen: Widgets.Screen): void {
