...
export function renderComponent(
  ...
  const screen = getScreen({
    ...
    mouse: options.screen?.mouse ?? true,
  ...
  try {
    ...
    const connectDOMToTerminal = (domNode: TerminalElementNode, parent: Widgets.Node): Widgets.Node => {
      ...
      if (!element) {
        console.warn(`[Renderer] Element ${tagName} has no associated terminal element`);
        return parent;
      }
    ...
    const rootElement = connectDOMToTerminal(target as TerminalElementNode, rootBox);
