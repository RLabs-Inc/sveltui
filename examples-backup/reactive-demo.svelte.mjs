...
Reactive_demo[$.FILENAME] = 'examples/reactive-demo.svelte';

import * as $ from 'svelte/internal/client';
import { onMount } from 'svelte';

var on_keydown = (key, increment, decrement) => {
	if ($.strict_equals(key, 'up')) increment(); else if ($.strict_equals(key, 'down')) decrement(); else if ($.strict_equals(key, 'q')) process.exit(0);
};

var root = $.add_locations($.template(`<box width="100%" height="100%" label=" Reactive Terminal Demo "><text left="center" content="SvelTUI Fine-Grained Reactivity Bridge"></text> <box label=" Counter "><text left="center"></text></box> <text left="center" content="Press ↑/↓ to change counter | q to quit"></text> <text></text></box> <box></box>`, 1), Reactive_demo[$.FILENAME], [
	[
		41,
		0,
		[
...
export default function Reactive_demo($$anchor, $$props) {
	$.check_target(new.target);
	$.push($$props, true, Reactive_demo);
	...
	var box_2 = $.sibling(box, 2);

	$.set_attribute(box_2, 'focused', true);
	box_2.__keydown = [on_keydown, increment, decrement];
	$.set_attribute(box_2, 'width', 0);
	$.set_attribute(box_2, 'height', 0);
